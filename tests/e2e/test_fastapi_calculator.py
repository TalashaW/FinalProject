...
# ==============================================================================
# Playwright UI Tests (BREAD Operations)
# ==============================================================================

def test_ui_user_registration(page, fastapi_server):
    """E2E: Test user registration via UI"""
...
def test_ui_user_login(page, fastapi_server):
    """E2E: Test user login via UI"""
    # First register
    unique_id = str(uuid4())[:8]
...
def test_ui_create_calculation_addition(page, fastapi_server):
    """E2E: Test creating addition calculation via UI"""
    ...
    from playwright.sync_api import expect
    expect(page.locator("table tbody")).to_contain_text("30")


def test_ui_view_calculation(page, fastapi_server):
    """E2E: Test viewing calculation details via UI"""
    # Setup: register, login, create calculation
    unique_id = str(uuid4())[:8]
    username = f"testuser_{unique_id}"
    password = "TestPass123!"
    ...
    page.fill("#calcInputs", "3, 4")
    page.click("#calculationForm button[type='submit']")
    page.wait_for_selector("#successAlert:not(.hidden)", timeout=5000)
    
    # Click View button
    page.click("table tbody tr:first-child a:has-text('View')")
    page.wait_for_url("**/dashboard/view/**", timeout=5000)
    
    # Verify details
    from playwright.sync_api import expect
    expect(page.locator("#calcDetails")).to_contain_text("multiplication")
    expect(page.locator("#calcDetails")).to_contain_text("12")


def test_ui_edit_calculation(page, fastapi_server):
    """E2E: Test editing calculation via UI"""
    # Setup: register, login, create calculation
    unique_id = str(uuid4())[:8]
    username = f"testuser_{unique_id}"
    password = "TestPass123!"
    ...
    # Click Edit button
    page.click("table tbody tr:first-child a:has-text('Edit')")
    ...
    from playwright.sync_api import expect
    expect(page.locator("#calcDetails")).to_contain_text("30")


def test_ui_delete_calculation(page, fastapi_server):
    """E2E: Test deleting calculation via UI"""
    # Setup: register, login, create calculation
    unique_id = str(uuid4())[:8]
    username = f"testuser_{unique_id}"
    password = "TestPass123!"
    ...
    # Click delete
    page.click("table tbody tr:first-child button:has-text('Delete')")
    
    # Wait a moment for deletion
    page.wait_for_timeout(1000)


def test_ui_invalid_inputs_negative(page, fastapi_server):
    """E2E NEGATIVE: Test invalid inputs show error"""
    # Setup: register and login
    unique_id = str(uuid4())[:8]
    username = f"testuser_{unique_id}"
    password = "TestPass123!"
    ...
    # Try invalid inputs
    page.select_option("#calcType", "addition")
    page.fill("#calcInputs", "abc, def")  # Invalid
    page.click("#calculationForm button[type='submit']")
...
def test_ui_division_by_zero_negative(page, fastapi_server):
    """E2E NEGATIVE: Test division by zero shows error"""
    # Setup: register and login
    unique_id = str(uuid4())[:8]
    username = f"testuser_{unique_id}"
    password = "TestPass123!"
    ...
    page.wait_for_selector("#errorAlert:not(.hidden)", timeout=5000)
    from playwright.sync_api import expect
    expect(page.locator("#errorMessage")).to_contain_text("zero")