...
def test_get_current_user_minimal_payload_with_sub(mock_verify_token):
    """Test the branch where token_data is a dict with only 'sub' key"""
    user_id = uuid4()
...
# NEW TEST: Test get_current_user with UUID directly
def test_get_current_user_uuid_payload(mock_verify_token):
    """Test the branch where token_data is directly a UUID"""
    user_id = uuid4()
...
# NEW TEST: Test get_current_user with unsupported token_data type
def test_get_current_user_unsupported_token_type(mock_verify_token):
    """Test the else branch where token_data is neither dict nor UUID"""
    # Return something unexpected like a string or list
    mock_verify_token.return_value = "some_string"
