...
def test_calculation_repr():
    """
    Test the __repr__ method of Calculation models.
    
    """
    dummy_user_id = uuid.uuid4()
    
    # Test Addition repr
    addition = Addition(user_id=dummy_user_id, inputs=[1, 2, 3])
    ...
    # Test Subtraction repr
    subtraction = Subtraction(user_id=dummy_user_id, inputs=[10, 5])
    repr_str = repr(subtraction)
    assert "Calculation" in repr_str
    assert "[10, 5]" in repr_str
    
    # Test that __repr__ is useful for debugging
    print(f"Addition object: {addition}")  # This will trigger __repr__
    print(f"Subtraction object: {subtraction}")
...
def test_abstract_calculation_get_result_not_implemented():
    ...
    # This is unusual - normally you'd only create subclasses
    base_calc = Calculation(
        user_id=dummy_user_id, 
        type="calculation",  # Use the base polymorphic_identity
        inputs=[1, 2, 3]
    )
    
    # Calling get_result on the base class should raise NotImplementedError
