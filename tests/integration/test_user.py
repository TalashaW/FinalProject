...
def managed_db_session():
    """
    Context manager for database sessions - for demonstration purposes.
    """
...
# ======================================================================================
# User.update() Method Tests
# ======================================================================================

def test_user_update_method(db_session, test_user):
    """
    Test User.update() method to cover.
    ...
    # Call the update method with multiple attributes
    ...
    logger.info("User.update() method test passed.")


def test_user_update_empty_kwargs(db_session, test_user):
    """Test update() with no parameters - should still update updated_at"""
    original_updated_at = test_user.updated_at
    original_email = test_user.email
    
    # Call update with no parameters - the for loop executes 0 times
    result = test_user.update()
    
    db_session.commit()
    db_session.refresh(test_user)
    
    # updated_at should still be refreshed
    assert test_user.updated_at > original_updated_at
    
    # Other fields should be unchanged
    assert test_user.email == original_email
    
    # Should still return self
    assert result is test_user


def test_user_update_returns_self(db_session, test_user):
    """Verify that update() returns the user instance"""
    result = test_user.update(first_name="Chained")
    
    # Should return the same instance
    assert result is test_user
    assert id(result) == id(test_user)
    
    # Can chain operations
    result2 = result.update(last_name="Operations")
    assert result2 is test_user
    assert test_user.last_name == "Operations"

# ======================================================================================
# hashed_password Property Test
# ======================================================================================

def test_hashed_password_property(db_session, test_user):
    """
    Test the hashed_password property getter.
    

    
    The @property decorator makes hashed_password act like an attribute,
    but it actually calls a method that returns self.password.
    """
    # The hashed_password property should return the same value as password
    assert test_user.hashed_password == test_user.password
    
    # They should be the exact same object
    assert test_user.hashed_password is test_user.password
    
    # Verify it's actually a hashed password (not plaintext)
    assert len(test_user.hashed_password) > 20  # Hashed passwords are long
    assert test_user.hashed_password != "password123"  # Not plaintext
    
    logger.info("hashed_password property test passed")


def test_hashed_password_property_after_update(db_session, test_user):
    """Verify hashed_password property works after password updates"""
    original_password = test_user.password
    
    # Update the password directly
    new_hashed = User.hash_password("NewPassword123")
    test_user.password = new_hashed
    
    db_session.commit()
    db_session.refresh(test_user)
    
    # hashed_password property should return the new password
    assert test_user.hashed_password == new_hashed
    assert test_user.hashed_password != original_password

# ======================================================================================
# verify_password Method Tests
# ======================================================================================

def test_verify_password_correct(db_session):
    """
    Test User.verify_password() with correct password.
    ...
    # Test with correct password - this calls verify_password method
    assert user.verify_password(plain_password) is True
    logger.info("verify_password with correct password - passed")


def test_verify_password_incorrect(db_session):
    """Test User.verify_password() with incorrect password"""
    ...
    logger.info("verify_password with incorrect password - passed")


def test_verify_password_case_sensitive(db_session):
    """Test that password verification is case-sensitive"""
    plain_password = "CaseSensitive123"
    hashed = User.hash_password(plain_password)
    ...
    # Correct case
    assert user.verify_password("CaseSensitive123") is True
    
    # Wrong case
    assert user.verify_password("casesensitive123") is False
